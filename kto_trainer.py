# Modified from KTOTrainer: https://github.com/huggingface/trl/blob/main/trl/trainer/kto_trainer.py
...
def _get_kl_dataset_slight_shuffle(batch: Dict[str, List[Any]]) -> Dict[str, List[Any]]:
    n = len(batch["answer_input_ids"])
    shuffle_factor = 2
    num_swaps = int(shuffle_factor * n)
    for _ in range(num_swaps):
        i = random.randint(0, n - 2)
        batch["answer_input_ids"][i], batch["answer_input_ids"][i + 1] = batch["answer_input_ids"][i + 1], batch["answer_input_ids"][i]
        batch["answer_attention_mask"][i], batch["answer_attention_mask"][i + 1] = batch["answer_attention_mask"][i + 1], batch["answer_attention_mask"][i]
    return batch
...
def _process_tokens(example: Dict[str, Any], model: "PreTrainedModel" = None, **kwargs) -> Dict:
    ...
    batch = {
        ...
        f"{kwargs['prefix']}weight": example["weight"],
    }
...
class WeightedKTOTrainer(Trainer):
    r"""
    ...
    def __init__(
            ...
            slight_shuffle: bool = None,
            weight: bool = None,
    ):
        self.slight_shuffle = slight_shuffle
        self.weight = weight
        ...
        with PartialState().local_main_process_first():
            ...
            if self.calculate_KL:
                ...
                train_kl_dataset = train_dataset.map(
                    _get_kl_dataset_slight_shuffle if self.slight_shuffle else _get_kl_dataset,
                    batched=True,
    ...
    def get_batch_loss_metrics(
        ...
        # weight losses
        if self.weight:
            chosen_idx = [i for i in range(len(batch["weight"])) if batch["label"][i] is True]
            rejected_idx = [i for i in range(len(batch["weight"])) if batch["label"][i] is False]
            weight=torch.tensor(batch["weight"])
            rearranged_weights = weight[chosen_idx+rejected_idx, ...]
            rearranged_weights = rearranged_weights.to(self.accelerator.device)
            losses = losses * rearranged_weights
