...
from rag.dataset.utils import get_dataset # load datasets
from rag.retrieval import ColbertApi
from rag.language_model import OpenaiModel
...
class NaiveRag:
    def __init__(self, args):
        ...
        # llm config
            #(other configs check rag/language_model/*.py files)
    ...
    def inference(self, query: Optional[str] = None, mode = 'interact'):
        ...
        if 'interact' == mode:
            ...
            def process_data(eval_data):
                ...
                # self.print_fn(f'{self.algorithm_name} {self.task} \n Question:{question} \n Rag Output:{outputs} \n Answers: {eval_data[self.EvalData.InputStruction.answer]}')
                return eval_data, outputs, time_use
            with ThreadPoolExecutor(20) as executor:
                results = list(tqdm(executor.map(process_data, self.eval_dataset), total=len(self.eval_dataset)))
            inference_results = []
            total_time = 0
            for eval_data, outputs, time_use in results:
            ...
            if self.task in ['ASQA','Factscore']:
                ...
                llm_score = self.EvalData.llm_judge(inference_results)
                avg_time = total_time / len(inference_results)
                self.print_fn(
                    f'{self.algorithm_name} {self.task} \n Accuracy: {acc} \n Exact match:{EM} \n F1 score: {f1_score} \n LLM score: {llm_score} \n Avg. time: {avg_time}')
                self.EvalData.save_result(inference_results)
                eval_result = {'Accuracy':acc, 'Exact match': EM, 'F1 score':f1_score, 'LLM score':llm_score, 'Avg. time': avg_time}
    ...
    def setup_logger(self, args):
        ...
        if args.llm_mode == 'HF_Model' or args.llm_mode == 'Lora_Model':
            ...
            model_name = os.path.basename(self.llm_name.rstrip('/'))
