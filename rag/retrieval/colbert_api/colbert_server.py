...
class ColbertServer:
    ...
    def setup_retrieve(self):
        ...
        try:
            ...
            print(
                f'{RED}Warning!!! Your wiki database has issues.{END}')
            user_input = input(f"Enter 'yes' to process data with Colbert Embedding, or 'no' to exit: ")
            if user_input.lower() == "yes":
                with Run().context(RunConfig(nranks=1,
                                             experiment=self.index_dbPath)):  # nranks specifies the number of GPUs to use.
                    ...
                    indexer.index(name=index_name, collection=collection,
                                  overwrite=True)  # overwrite must in [True, False, 'reuse', 'resume', "force_silent_overwrite"]
    ...
    def api_search_query(self, query, k):
        ...
        for passage_id, passage_rank, passage_score in zip(*ids):
            if '|' in self.searcher.collection[passage_id]:
                title, content = self.searcher.collection[passage_id].split('|',
                                                                            1)  # max split is 1. The first place is title
                passages[passage_rank] = {'id': passage_id, 'title': title.strip(), 'text': content.strip(),
                                          'score': passage_score}
            else:
                passages[passage_rank] = {'id': passage_id, 'text': self.searcher.collection[passage_id],
                                          'score': passage_score}
        return passages
